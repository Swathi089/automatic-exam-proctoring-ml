...
export default function ExamCompleted() {
  ...
  const [submissionTime] = useState<string>(new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }));

  useEffect(() => {
    // Get exam title from localStorage or API
    const selectedExamId = localStorage.getItem("selectedExamId");
    if (selectedExamId) {
      // Try to fetch exam data
      fetch(`/api/exam/${selectedExamId}`)
        ...
        .catch(() => {
          // Keep default "Exam" title on error
        });
    }
  }, []);
        ...
        <div className="bg-white rounded-2xl shadow-lg p-12 mb-8 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{submissionTime}</p>
            </div>
            <div>
